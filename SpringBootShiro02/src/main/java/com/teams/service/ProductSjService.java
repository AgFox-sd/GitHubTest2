...
public interface ProductSjService {
	...
	// 查询审核通过的产品信息
	PageInfo<D_file> selectProduct(Params params);
	...
	PageInfo<D_file> selectcpdash(Params params);// 产品档案等待审核查询
	...
	PageInfo<D_module> selectwusjsh(Params params);// 产品物料设计单查询
	...
	int delwlsj(String check_tag,String check_yj,String product_id,String design_id);// 物料组成设计不通过

	// 查询显示的信息为所有已通过复核的产品档案
	PageInfo<D_file> selectId2(Params params);
	...
	// 查询通过审核的商品档案信息
	PageInfo<D_file> Selad(Params params);
	...
	// 物料查询
	PageInfo<D_module> selD_module(Params params);
	...
	PageInfo<D_module> selD_moduleS(Params params);

	int delwlsjzcd(String design_id, String product_id);

	int selectCount(String sjdh);

	List<D_file> Selad2(String string, String string2, int i, String sjdh);

	void updatezje(double subtotal, String design_id);
	
	void updatejzje(double subtotal, String design_id);
}
