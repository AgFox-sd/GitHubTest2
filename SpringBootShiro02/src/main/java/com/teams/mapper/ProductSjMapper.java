...
public interface ProductSjMapper {
	...
	//查询审核通过的产品信息
	List<D_file> selectProduct(Params params);
	...
	List<D_file> selectcpdash(Params params);//产品档案等待审核查询
	...
	List<D_module> selectwusjsh(Params params);//产品物料设计单查询
	...
	List<D_file> selectId2(Params params);
	//通过产品编号(id)查询档案信息
	...
	//查询通过审核的商品档案信息
	List<D_file> Selad(Params params);
    ...
    //物料查询
    List<D_module> selD_module(Params params);
 ...
 // 物料明细单
	...
	List<D_module> selD_moduleS(Params params);

	int delwlsjzcd(String design_id, String product_id);

	int selectCount(String sjdh);

	List<D_file> Selad2(String check_tag, String design_module_tag, int tid, String sjdh);
